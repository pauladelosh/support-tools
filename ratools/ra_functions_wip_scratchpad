...
# RA_INITIALS="XYZ"
# source ~/<path-to-support-tools-repo>/ratools/ra_functions.sh
#
# Instructions:
          ...
          case $yn in
              [Yy]* ) cd $modpath
                ...
                if [ $? -ne 1 ]
                  then
                    svn rm "$1"
                    #svn commit -m "$RA_INITIALS@Acquia, Ticket #$4: Module Update, cleanup, removing $1-$2 at $modpath."
                    if [ "$5" = "--security" ]
                    ...
                    svn add --force "$1"
                    #svn commit -m "$RA_INITIALS@Acquia, Ticket #$4: Module Update, updating $1-$3 at $modpath from $2."
                    if [ "$5" = "--security" ]
...
curl "http://ftp.drupal.org/files/projects/$1-$2.tar.gz" | tar xz
#module-cache-check $1 $2
#cp -R ~/Sites/releases/modules/$1/$2/$1 .
git add "$1"
